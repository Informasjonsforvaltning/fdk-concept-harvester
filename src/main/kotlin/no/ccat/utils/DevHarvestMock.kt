...
fun startMockServer() {
    if(!mockserver.isRunning) {
        ...
        mockserver.stubFor(get(urlEqualTo("/api/organizations/12345678"))
                .willReturn(okJson(File("src/test/resources/contract/org-0.json").readText())))
        mockserver.stubFor(get(urlEqualTo("/api/organizations/961181399"))
                .willReturn(okJson(File("src/test/resources/contract/org-1.json").readText())))
        mockserver.stubFor(get(urlEqualTo("/api/organizations/974761076"))
                .willReturn(okJson(File("src/test/resources/contract/org-1.json").readText())))
        mockserver.stubFor(get(urlEqualTo("/api/organizations/974760673"))
                .willReturn(okJson(File("src/test/resources/contract/org-1.json").readText())))
