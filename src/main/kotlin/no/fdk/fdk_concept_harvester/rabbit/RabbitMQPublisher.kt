...
import no.fdk.fdk_concept_harvester.model.UpdateSearchMessage
...
class RabbitMQPublisher(private val template: RabbitTemplate) {
    fun send(dbId: String?) {
        try {
            template.convertAndSend("harvests","concepts.harvester.UpdateSearchTrigger", UpdateSearchMessage(dbId))
            template.convertAndSend("harvests","concepts.harvested", UpdateSearchMessage(dbId))
